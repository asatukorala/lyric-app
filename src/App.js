...
class App extends React.Component {
  ...
  handleLyricDisplay = event => {
   // const [lyrics, trackID] = React.useState(null)
    ...
    const res = fetch("http://api.musixmatch.com/ws/1.1/track.search?apikey=140e78b9724d3b447cc2cbee1a92bf2f&q_artist=toto&q_track=rosanna")
      .then(res => res.json())
      .then(res => console.log(res['message']['body']['track_list'][0]['track']['track_id']))
      // .then(res => this.state.trackID = res['message']['body']['track_list'][0]['track']['track_id'])
      .then(res => console.log(this.state.trackID))
  ...
  render() {
    ...
    return (
      <div className="App">
        <LyricDisplay
          handleLyricDisplay={this.handleLyricDisplay}
          lyrics={lyrics}
        />
      </div>
    )
